...
def main():
    ...
    wav2vec2_config = Wav2Vec2Config.from_pretrained(args.model_name_or_path, output_attentions=True)
    processor = Wav2Vec2Processor.from_pretrained(args.model_name_or_path, config=wav2vec2_config)
    ...
    print(f'wav2vec2_config: {wav2vec2_config}')
    ...
    for i, split_name in enumerate(split_names):
        ...
        wandb_logger.log_metrics(metric, step=trainer.state.global_step)
    print(f"metrics: {metrics}")
    wandb_logger.log_metrics(metrics, step=trainer.state.global_step)
