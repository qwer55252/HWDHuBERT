    ...
    def __call__(self, features):
        ...
        with self.processor.as_target_processor():
            if "text" in features[0].keys():
                ...
                texts = [
                    f["text"] if f["text"] != "ignore_time_segment_in_scoring" else ""
                    for f in features
                ]
                # processor의 __call__을 사용하면 내부에서 tokenizer를 호출하여 적절히 처리됨
                labels = self.processor.tokenizer(
                    texts, padding=True, return_tensors="pt"
                )
                # pad 토큰을 -100으로 치환하여 loss 계산시 무시
...
def preprocess_function(batch):
    ...
    if batch["text"] and isinstance(batch["text"], list): # batch["text"] 가 list이면
        ...
        # 어떻게 처리할까
        print(f"Unsupported text format: {type(batch['text'])}")
        return None
    return batch
...
def main():
    ...
    test_dataset = load_dataset("./tedlium_test.py", "release1", split=args.test_split, trust_remote_code=True) # test-clean, test-other 둘 다 사용할 경우
